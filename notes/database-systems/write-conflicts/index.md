...
Conflict avoidance arranges writes so that conflicting operations can't be accepted independently.
...
Conflict avoidance is often preferable when its availability and latency costs are acceptable. It isn't always possible in highly available, multi-writer, peer-to-peer, or offline-first systems.
...
LWW is simple and ensures that replicas can make the same deterministic choice. However, it doesn't truly merge concurrent information. One write is retained and the others are discarded, even if the information could've or should've been merged.

Consequently, LWW implies data loss.

### Physical-Clock Problems

Using [[Time|physical timestamps]] assumes that clocks are sufficiently synchronized. In practice, timestamps aren't entirely reliable.
...
Some databases and replication systems support *application-defined merging*, which allows applications to provide custom conflict-resolution logic.
...
Manual conflict-resolution code should be minimized where possible because there are many edge cases to handle. For additional context, see [this discussion of application-defined conflict resolution](https://youtu.be/DEcwa68f-jY?si=ZQeotVLEiZ995f63&t=696).
...
*Operational Transformation* (OT) is a family of algorithms primarily associated with collaborative editing.
...
When concurrent operations are created against the same earlier document state, applying one may invalidate the coordinates used by the other. OT transforms an operation to account for the effects of the operations applied before it.
...
OT isn't a single algorithm but a family of algorithms. A particular OT implementation must define:
...
OT algorithms are extremely difficult to implement because developers must write transformation rules for every possible combination of operations. OT is also prone to edge cases and is difficult to prove correct.

> Many academic OT algorithms that were thought to be correct were later proven to cause divergence under edge cases. See [this discussion of OT correctness](https://youtu.be/B5NULPSiOGw?si=ZsT91IcrRJugoblj&t=732).
...
Thus, many practical OT systems rely on a central server (a star topology), which brings related drawbacks such as a single point of failure and scalability bottlenecks.

## Conflict-Free Replicated Data Types

A *conflict-free replicated data type* (CRDT) is a replicated data type whose update and merge rules are designed so that replicas can be updated independently and later converge deterministically.
...
*State-based CRDTs* (also called convergent replicated data types or CvRDTs) maintain state at each replica and merge states received from other replicas. They require:
...
*Operation-based CRDTs* (also called commutative replicated data types or CmRDTs) broadcast operations instead of full state.
...
> In theory, state-based and operation-based CRDTs are equivalent because they can emulate each other. In practice, state-based CRDTs are easier to implement, but transmitting full state is more expensive; operation-based CRDTs are lightweight to transmit but require stricter guarantees from communication middleware.

#### Counterexamples

Without exactly-once delivery, deduplication, or an idempotent representation (a violation of the non-duplication requirement):

1. The states of replicas A and B are both 0.
2. Replica A broadcasts `increment(1)`.
3. Replica B receives the operation twice and applies it.
4. The state of replica A is 1, while the state of replica B is 2. This causes divergence.

With dropped operations (no retry or recovery system), the opposite case occurs:

1. The states of replicas A and B are both 0.
2. Replica A broadcasts `increment(1)`.
3. The operation is dropped, so replica B doesn't receive anything.
4. The state of replica A is 1, while the state of replica B is 0. This causes divergence.

Without causal ordering:

1. The states of replicas A and B are both `{}` (the empty set).
2. Replica A broadcasts `add("foo")`, then broadcasts `remove("foo")`.
3. Replica B receives them in a non-causal order: `remove("foo")`, then `add("foo")`.
4. The state of replica A is `{}`, while the state of replica B is `{"foo"}` because removing a nonexistent element is a no-op.
...
Removing an element adds it to `R`. Because `R` never shrinks, a removed element can't be added again using the same element identity.
...
An LWW-Element-Set can trivially be extended to become an LWW-Element-Map by storing a tuple of `(value, timestamp)`:

```text
theme: ("dark", timestamp=20)
```

However, other set CRDTs can't easily be converted into maps. This is because a set only answers "is this element present?", whereas a map needs to answer two questions: "is this key present?" and "what value is associated with this key?" Those require additional conflict semantics.
...
Set union is commutative, but array insertion by numerical index isn't. Thus, sequence CRDT design is difficult and remains an area of research.
